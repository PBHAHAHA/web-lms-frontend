    ...
    <div class="bg-white dark:bg-zinc-900 shadow p-4 md:p-8 mb-8">
      <div class="flex flex-col md:flex-row gap-4">
        <div class="w-full md:w-48 md:h-32 relative bg-gray-100 dark:bg-gray-800 rounded overflow-hidden">
          ...
          <!-- 占位图 -->
          <div 
            v-if="imageError || !courseInfo?.img" 
            class="absolute inset-0 flex items-center justify-center bg-black text-white px-4"
          >
            <span class="text-sm text-center font-medium">
              {{ courseInfo?.title || '课程封面' }}
            </span>
          </div>
        </div>
        <div class="flex-1">
          ...
          <p class="text-sm text-muted-foreground leading-relaxed">
            <div v-html="courseInfo?.described || ''"></div>
          </p>
          <!-- 课程标签 -->
